...
def embedding(x, number_classes, embedding_size, name='embedding'):
  """Creates a spectral normalized embedding lookup layer.
...
class Im2NetCondBN(object):
  """Conditional BatchNorm.

  For each  class, it has a specific gamma and beta as normalization variable.
  """

  def __init__(self, name='conditional_batch_norm', decay_rate=0.999, center=True,
               scale=True):
    with tf.variable_scope(name):
      self.name = name
      self.center = center
  ...
  def __call__(self, inputs, beta_in, gamma_in, labels, is_training=True):
    inputs = tf.convert_to_tensor(inputs)
    inputs_shape = inputs.get_shape()
    params_shape = inputs_shape[-1:]
    num_categories = labels.get_shape()[-1]
    axis = [0, 1, 2]
    ...
    with tf.variable_scope(self.name):
      ...
      print('self.beta: {}'.format(self.beta.get_shape().as_list()))
      print('self.gamma: {}'.format(self.gamma.get_shape().as_list()))
      print('beta_in: {}'.format(beta_in.get_shape().as_list()))
      print('labels: {}'.format(labels.get_shape().as_list()))

      beta = tf.expand_dims(tf.expand_dims(tf.matmul(labels, self.beta), 1), 1)
      print('labels mul self.beta: {}'.format(beta.get_shape().as_list()))

      beta += beta_in
      gamma = tf.expand_dims(tf.expand_dims(tf.matmul(labels, self.gamma), 1), 1)

      gamma += gamma_in
      decay = self.decay_rate
      variance_epsilon = 1E-5

      print('beta: {}'.format(beta.get_shape().as_list()))
      print('gamma: {}'.format(gamma.get_shape().as_list()))
...
class Im2NetBN(object):
  ...
  def __call__(self, inputs, beta, gamma, is_training=True):
    inputs = tf.convert_to_tensor(inputs)
    inputs_shape = inputs.get_shape()
    params_shape = inputs_shape[-1:]
    axis = [0, 1, 2]
    ...
    with tf.variable_scope(self.name):
      ...
      beta = beta
      gamma = gamma+tf.ones_like(gamma)
      decay = self.decay_rate
      variance_epsilon = 1E-5
      if is_training:
        mean, variance = tf.nn.moments(inputs, axis, keep_dims=True)
